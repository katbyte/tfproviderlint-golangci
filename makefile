...
depscheck: ## Check that go.mod/go.sum are in sync
	@echo "==> Checking source code with go mod tidy..."
	@go mod tidy
	@git diff --exit-code -- go.mod go.sum || \
		(echo; echo "Unexpected difference in go.mod/go.sum files. Run 'go mod tidy' and commit."; exit 1)

check-all: test lint depscheck actionlint yamllint shellcheck typos zizmor ## Run every check


.PHONY: default all help tools fmt goimports lint lint-fix actionlint yamllint shellcheck typos typos-fix zizmor test depscheck check-all
